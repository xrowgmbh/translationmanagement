...
{if ezhttp_hasvariable('RedirectURIAfterPublish', 'session')}
    {if ezhttp('RedirectURIAfterPublish', 'session')|contains('nopublish')}
        {set $nopublish = true()}
    {/if}
{/if}
{if $nopublish}
    <input class="defaultbutton" type="submit" name="StoreExitButton" value="{'Store draft and exit'|i18n( 'design/admin/content/edit' )}" title="{'Store the draft that is being edited and exit from edit mode. Use when you need to exit your work and return later to continue.'|i18n( 'design/admin/content/edit' )}" />  
{else}
